...
def drawSceneGraphNode(node, pipeline, transformName, parentTransform=tr.identity()):
    ...
    # agregué esto para instanciar mi nave que separé en cuerpo, propulsores y armas
    if len(node.childs) == 3 and isinstance(node.childs[0], gs.GPUShape):
        if isinstance(node.childs[1], gs.GPUShape) and isinstance(node.childs[2], gs.GPUShape):
            for leaf in node.childs:
    ...
    elif len(node.childs) == 1 and isinstance(node.childs[0], gs.GPUShape):
        leaf = node.childs[0]
        glUniformMatrix4fv(glGetUniformLocation(pipeline.shaderProgram, transformName), 1, GL_TRUE, newTransform)
        pipeline.drawCall(leaf)
